...
class UNet2DSimpleConditionModel(UNet2DModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # If in_channels not explicitly provided, calculate it
        if in_channels is None and sample_channels is not None and condition_channels is not None:
    ...
    def forward(self, sample, timestep, encoder_hidden_states=None, **kwargs):
        ...
        if encoder_hidden_states is not None:
            ...
            combined_input = torch.cat([sample, encoder_hidden_states], dim=1)
        else:
            combined_input = sample
            
        # Call parent forward
        return super().forward(combined_input, timestep, **kwargs)


class UNet2DFlexibleConditionModel:
    """
    ...
    def from_config(cls, config):
        ...
        Returns:
            ...
            UNet2DSimpleConditionModel (for concatenation-based conditioning)
        """
        # Extract mode from config, default to cross_attention
        mode = config.pop("mode", "concat")
        ...
        if mode == "concat":
            # For concat mode, create a UNet2DSimpleConditionModel
            ...
            # Create and return UNet2DSimpleConditionModel
            return UNet2DSimpleConditionModel(**config_copy)
        else:
