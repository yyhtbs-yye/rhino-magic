...
from rhino.adversarials.discriminators import VAEDiscriminator
...
class VAEWithAdversarial(pl.LightningModule):
    ...
    def __init__(self, model_config, train_config, validation_config):
        ...
        vae_optimization_config = train_config.get('vae_optimizer', {})
        disc_optimization_config = train_config.get('disc_optimizer', {})
        # Training params
        self.vae_steps = vae_optimization_config.get('vae_steps', 1)
        self.disc_steps = disc_optimization_config.get('disc_steps', 1)
        self.vae_use_ema = vae_optimization_config.get('use_ema', True)
        self.vae_ema_decay = vae_optimization_config.get('ema_decay', 0.999)
        self.vae_ema_start = vae_optimization_config.get('ema_start', 1000)
        
        # Extract learning rates and betas from the config
        self.vae_learning_rate = vae_optimization_config.get('learning_rate', 0.0001)
        self.disc_learning_rate = disc_optimization_config.get('learning_rate', 0.0002)
        self.vae_betas = vae_optimization_config.get('betas', [0.5, 0.999])
        self.disc_betas = disc_optimization_config.get('betas', [0.5, 0.999])
        self.vae_weight_decay = vae_optimization_config.get('weight_decay', 0.01)
        self.disc_weight_decay = disc_optimization_config.get('weight_decay', 0.01)
