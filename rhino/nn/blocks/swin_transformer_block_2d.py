...
from rhino.nn.minimals.pe.relative_position_bias_2d import RelativePositionBias2D
...
class SwinTransformerBlock2D(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, window_size, patch_size, 
        ...
        self.shift_size = (window_size[0] // 2, window_size[1] // 2) if trigger else (0, 0)
        ...
        self.rpe = RelativePositionBias2D(window_size=self.window_size, 
    ...
    def forward(self, x, cond=None): 
        ...
        if self.trigger:
            skey = (H, W, *self.window_size, *self.shift_size)
            if skey not in self.attn_mask:
                self.attn_mask[skey] = masking.compute_mask_2d((H, W), tuple(self.window_size), self.shift_size).to(x.device)
        ...
        x = windowing.window_partition_2d(x, self.window_size) 
        
        rpe = self.rpe()
        x = self.attn(x, mask=mask, rpe=rpe)

        x = windowing.window_reverse_2d(x.view(-1, *self.window_size, C), self.window_size, B, H, W)
