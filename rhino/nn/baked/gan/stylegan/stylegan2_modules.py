...
from rhino.nn.baked.bcv.fused_bias_leakyrelu import (FusedBiasLeakyReLU, fused_bias_leakyrelu)
from rhino.nn.baked.bcv.upfirdn2d import upfirdn2d

from rhino.nn.baked.gan.pggan.pggan_modules import EqualizedLRConvModule, equalized_lr
from rhino.nn.baked.gan.stylegan.stylegan1_modules import Blur, EqualLinearActModule, NoiseInjection, make_kernel
...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, x, style, input_gain=None):
        ...
        # if x.dtype == CAST_DTYPE and self.demodulate:
        ...
        with torch.autocast(device_type="cuda", dtype=CAST_DTYPE, enabled=self.fp16_enabled):
            # process style code
...
class ModMBStddevLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Reshape to [G, M, Gc, C', H, W]
        #   where M = N // G, Gc = channel_groups, C' = C // Gc
        y = torch.reshape(x, (group_size, -1, self.channel_groups,
                              c // self.channel_groups, h, w))
        # Variance across the group dimension (G)
        y = torch.var(y, dim=0, unbiased=False)
        y = torch.sqrt(y + self.eps)
        
        # Average over channels & spatial dims to get one scalar per (M, Gc)
        y = y.mean(dim=(2, 3, 4), keepdim=True).squeeze(2)
        # Tile back over the group dimension to match batch N = G * M
